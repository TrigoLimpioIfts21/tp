...
class Items_web extends Secure_area implements iData_controller
{
	...
	{
		parent::__construct('items_web');
	}

	function index()
	{
		$config['base_url'] = site_url('/items_web/index');
	...
	function refresh()
	{
		...
		$data['controller_name']=strtolower(get_class());
		$data['form_width']=$this->get_form_width();
		$data['manage_table']=get_items_manage_tablew($this->Item->get_all_filtered($low_inventory,$is_serialized,$no_description),$this);
		$this->load->view('items_web/manage',$data);
	}
	...
	function search()
	{
		...
		$data_rows=get_items_manage_table_data_rowsw($this->Item->search($search),$this);
		echo $data_rows;
	}
	...
	*/
	...
	function view($item_id=-1)
	{
		...
		$this->load->view("items_web/form",$data);
	}
	...
	{
		$data['item_info']=$this->Item->get_info($item_id);
		$this->load->view("items_web/inventory",$data);
	}
	
	function count_details($item_id=-1)
	{
		$data['item_info']=$this->Item->get_info($item_id);
		$this->load->view("items_web/count_details",$data);
	} //------------------------------------------- Ramel
	...
	function bulk_edit()
	{
		$data = array();
		$suppliers = array('' => $this->lang->line('items_none'));
		foreach($this->Supplier->get_allw()->result_array() as $row)
		{
		...
		$this->load->view("items_web/form_bulk", $data);
	}
