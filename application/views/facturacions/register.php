...
<div id="page_title" style="margin-bottom:8px;"><?php echo $this->lang->line('invoice_register'); ?></div>
<?php

/*
PENDIENTE:
CHECKBOX ---->ENTREGADO
CORTE EN PEDIDO----FACTURA
*/
...
<?php echo form_open("facturacion/register.php/change_mode",array('id'=>'mode_form')); ?>
	<span><?php echo $this->lang->line('sales_mode') ?></span>
<?php echo form_dropdown('mode',$modes,$mode,'onchange="$(\'#mode_form\').submit();"'); ?>
<div id="show_suspended_sales_button">
	<?php echo anchor("facturacions/suspended/width:425",
...
<?php echo form_open("facturacions/add",array('id'=>'add_item_form')); ?>
...
else
{
	foreach(array_reverse($cart, true) as $line=>$item)
	{
		...
		echo form_open("facturacions/edit_item/$line");
	?>
		<tr>
		<td><?php echo anchor("facturacions/delete_item/$line",'['.$this->lang->line('common_delete').']');?></td>
...
<div id="overall_sale">
	...
	if(isset($customer))
	{
		...
		echo anchor("facturacions/remove_customer",'['.$this->lang->line('common_remove').' '.$this->lang->line('customers_customer').']');
	}
	else
	{
		echo form_open("facturacions/select_customer",array('id'=>'select_customer_form')); ?>
	...
	?>
    	...
    	<div id="Cancel_sale">
		<?php echo form_open("facturacions/cancel_sale",array('id'=>'cancel_sale_form')); ?>
		...
		if(count($payments) > 0 & ($entregado) == 1)
		{
		?>
			<div id="finish_sale">
				<?php echo form_open("facturacions/complete",array('id'=>'finish_sale_form')); ?>
    ...
    <table width="100%"><tr>
    <td style="width:55%; "><div class="float_left"><?php echo 'Total de Pagos:' ?></div></td>
	...
	<tr>
	<td style="width:55%; "><div class="float_left" ><?php echo 'Monto a Pagar:' ?></div></td>
	...
	<div id="Payment_Types" >
		...
		<div style="height:100px;">
			...
			<?php echo form_open("facturacions/add_payment",array('id'=>'add_payment_form')); ?>
			<table width="100%">
			<tr>
			<td>
		...
		?>
			...
			<?php
				...
				{
				echo form_open("facturacions/edit_payment/$payment_id",array('id'=>'edit_payment_form'.$payment_id));
				?>
	            <tr>
	            <td><?php echo anchor( "facturacions/delete_payment/$payment_id", '['.$this->lang->line('common_delete').']' ); ?></td>
...
{
    $("#item").autocomplete('<?php echo site_url("facturacions/item_search"); ?>',
    {
    	minChars:0,
    	max:100,
    	selectFirst: false,
    ...
    $("#customer").autocomplete('<?php echo site_url("facturacions/customer_search"); ?>',
    {
    	minChars:0,
    	delay:10,
    	max:100,
	...
	{
		$.post('<?php echo site_url("facturacions/set_comment");?>', {comment: $('#comment').val()});
	});
	
	$('#entregado').change(function() 
	{
		$.post('<?php echo site_url("facturacions/set_entregado");?>', {entregado: $('#entregado').is(':checked') ? '1' : '0'});
		
	});
	
	$('#email_receipt').change(function() 
	{
		$.post('<?php echo site_url("facturacions/set_email_receipt");?>', {email_receipt: $('#email_receipt').is(':checked') ? '1' : '0'});
	});
	...
	$("#suspend_sale_button").click(function()
	{
    	...
    	{
			$('#finish_sale_form').attr('action', '<?php echo site_url("facturacions/suspend"); ?>');
    		$('#finish_sale_form').submit();
    	}
	});
